...
//			printf("File:\n%s\n", buffer);
			...
			char index[MAXPATH];
			sprintf(index, "%s/index.html",rootPath);
...
void *ThreadProcess(void *threadid){
    ...
    for(;;){
		...
		std::cout << "GOT socket " << hSocket <<std::endl;
...
int main(int argc, char* argv[])
{
    ...
    int nHostPort;
	if(argc < 4){
        printf("\nUsage: server host-port thread-count dir\n");
        return 0;
    }
	else{
        ...
        rc = pthread_create(&threads[t], NULL, ThreadProcess, (void *)t);
 	       if(rc){
